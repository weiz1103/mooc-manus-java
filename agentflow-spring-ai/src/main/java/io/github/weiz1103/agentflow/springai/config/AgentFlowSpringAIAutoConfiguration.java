...
import io.github.weiz1103.agentflow.springai.domain.port.*;
import io.github.weiz1103.agentflow.springai.memory.AgentMemoryStore;
import io.github.weiz1103.agentflow.springai.memory.InMemoryAgentMemoryStore;
import io.github.weiz1103.agentflow.springai.session.InMemorySessionStateLoader;
...
/**
 * agentflow-spring-ai 模块。Spring Boot 自动配置。
 * <p>
 * 遵循 Spring Boot 条件装配原则。
 * <ul>
 *   <li>基础组件（内存实现）使用 {@code @ConditionalOnMissingBean} 提供默认实现。
 *       方便 agentflow-api 层替换为持久化实现。</li>
 *   <li>工具服务（FileToolService 等）在对应的 Port Bean 存在时自动注册。</li>
 *   <li>ToolCallbackRegistry 汇总所有工具，。SpringAIPlannerReActFlow 使用。</li>
 * </ul>
 * </p>
 *
 * <h3>扩展方式</h3>
 * <ol>
 *   <li>。agentflow-api 层注。{@link SandboxPort}、{@link BrowserPort}。
 *       {@link SearchEnginePort} 的具体实。Bean，本配置将自动生效。</li>
 *   <li>如需替换内存实现，在 agentflow-api 中注册相同类型的 Bean。
 *       {@link AgentMemoryStore} / {@link SessionStateLoader}）即可。</li>
 *   <li>如需禁用某个工具，在 agentflow-api 中排除对应的 Port Bean 即可。</li>
 * </ol>
 */
@Configuration
@EnableConfigurationProperties(AgentFlowAgentProperties.class)
public class AgentFlowSpringAIAutoConfiguration {

    // ─────────────────────────────────────────────
    // 默认基础设施实现（可。agentflow-api 替换。
    // ─────────────────────────────────────────────

    /**
     * 默认内存 AgentMemoryStore（生产环境由 agentflow-api 替换。JPA 实现）。
     */
    @Bean
    ...
    /**
     * 默认内存 SessionStateLoader（生产环境由 agentflow-api 替换。JPA 实现）。
     */
    @Bean
     ...
     * 工具回调注册。Bean。
     * <p>
     * 注意：至少需。FileToolService + ShellToolService + BrowserToolService +
