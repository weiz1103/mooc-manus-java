...
public class SessionCommandService {
    ...
    public void delete(String sessionId) {
        ...
        log.info("会话已删。 sessionId={}", sessionId);
    }
    ...
    public void stop(String sessionId) {
        ...
        log.info("会话已停。 sessionId={}", sessionId);
    }
    ...
    private void recordToTaskStream(io.github.weiz1103.agentflow.api.domain.external.Task task, BaseEvent event) {
        try {
            String id = task.getOutputStream().put(JSON.writeValueAsString(event));
            event.setId(id);
        } catch (Exception e) {
            log.debug("停止时写入终止事件失。 {}", e.getMessage());
        }
    }
}
