 ...
 * <p>
 * 设计要点：
 * - 使用 Spring AI ChatClient 替代自定义 LLM 接口
...
public abstract class BaseSpringAIAgent {
     ...
     * <p>
     * 完整流程：
     * 1. 从 memoryStore 加载历史记忆（含 system prompt）
     * 2. 将新的 user 消息添加到记忆并持久化
         * 3. 调用 ChatClient（internalToolExecutionEnabled=false，禁止自动执行工具）
     * 4. 获取 AssistantMessage，判断是否有工具调用
     * 5. 若有工具调用：发送 ToolEvent(CALLING) → 执行工具 → 发送 ToolEvent(CALLED) → 添加工具响应到记忆 → 循环
     * 6. message_ask_user 工具调用时：发送 MessageEvent → 发送 WaitEvent → 返回 null（中断）
     * 7. 达到最大迭代次数时：发送 ErrorEvent
     * 8. 无工具调用时：发送 MessageEvent（最终内容）并返回
     * </p>
     *
    ...
    protected AssistantMessage invokeLlm(
        ...
        OpenAiChatOptions.Builder optionsBuilder = OpenAiChatOptions.builder()
                ...
                .internalToolExecutionEnabled(false); // 禁用自动工具执行，手动控制工具调用循环
        ...
        if ("none".equals(toolChoice)) {
            // PlannerAgent: 不允许调用工具，只输出 JSON 内容
        ...
        while (iterations < config.maxIterations()) {
            ...
            // 5.5 特殊处理 message_ask_user 和 message_notify_user（直接发送 MessageEvent，跳过 ToolEvent 以防重复）
            ...
            // 5.9 特殊处理 message_ask_user（CALLED 时发送 WaitEvent 并中断流程）
            if ("message_ask_user".equals(functionName)) {
                emitter.accept(WaitEvent.builder().build());
                return null; // 返回 null 信号：等待用户输入
            }
        ...
        // 6. 达到最大迭代次数
        logger.warn("Agent[{}] 会话[{}] 迭代超过最大次数: {}", getName(), sessionId, config.maxIterations());
        emitter.accept(ErrorEvent.builder()
                .error("Agent迭代超过最大迭代次数: " + config.maxIterations() + ", 任务处理失败")
                .build());
        return null;
    }
    ...
    private ChatResponse callWithRetry(List<Message> messages, OpenAiChatOptions options, Consumer<BaseEvent> emitter) {
        ...
        for (int i = 0; i < config.maxRetries(); i++) {
            try {
                ...
                if (output.getText() == null
                    ...
                    logger.warn("LLM 返回了空内容，执行重试 ({}/{})", i + 1, config.maxRetries());
        ...
        logger.error("调用 LLM 失败，已达到最大重试次数: {}", config.maxRetries());
        if (lastError != null) {
            emitter.accept(ErrorEvent.builder()
    ...
    public void rollBack(AgentMessage message) {
        ...
        // 1. 检查最后一条消息是否是工具调用的 assistant 消息
        ...
        // 3. 如果是 message_ask_user，添加工具响应后继续（用户已回复）
        if ("message_ask_user".equals(functionName)) {
            try {
                ...
                logger.error("rollBack 序列化失败: {}", e.getMessage());
            }
        } else {
            // 4. 其他工具调用：直接删除最后一条 assistant（工具调用）消息
    ...
    /**
     * 将 OpenAI 原始格式消息列表转换为 Spring AI Message 列表。
     *
    ...
    /**
     * 将 Spring AI AssistantMessage 转换为 OpenAI 原始格式 Map。
     *
