 ...
 * <p>
 * 设计原则。
 * - 消息存储。List&lt;Map&lt;String, Object&gt;&gt;（OpenAI兼容原始格式。
 * - 键为 sessionId + agentName 的组合，确保每个会话每个Agent独立存储
 * - 默认实现为内存存储，生产环境通过 JpaAgentMemoryStore 持久化到数据。
 * </p>
 *
...
public interface AgentMemoryStore {
    ...
    /**
     * 加载指定会话+Agent的历史消息列表。
     * 消息格式。OpenAI API 原始格式（role/content/tool_calls 等字段）。
     *
     * @param sessionId 会话id
     * @param agentName Agent名称。planner" 。"react"。
    ...
    /**
     * 压缩指定Agent的记忆，移除重量级内容（浏览器页面内容等），以减。Token 消耗。
     * 默认实现为空操作，子类可按需重写。
     *
     * @param sessionId 会话id
     * @param agentName Agent名称
    ...
    default void compact(String sessionId, String agentName) {
        ...
        for (Map<String, Object> message : messages) {
            ...
            // 2. 移除 reasoning_content（DeepSeek思考链内容），避免重复占用上下。
