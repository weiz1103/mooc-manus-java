...
import io.github.weiz1103.agentflow.api.application.service.SessionQueryService;
import io.github.weiz1103.agentflow.api.application.service.SessionCommandService;
...
public class SessionController {
     ...
     * <p>两种场景共用此接口：
     * <ul>
     *   <li>携带 {@code message}：发送新消息，触。Agent 执行</li>
     *   <li>不携。{@code message}：断线续流，。{@code event_id} 断点重放</li>
     * </ul>
     * </p>
     */
    ...
    public SseEmitter chat(@PathVariable String sessionId,
        ...
        // 虚拟线程处理阻塞。AI 执行过程，不阻塞 Tomcat 线程
        ...
        Thread.ofVirtual().name("session-stream").start(() -> {
            while (!publisher.isClientGone()) {
                try {
                    List<Map<String, Object>> items = sessionQueryService.listAll().stream()
                            .map(this::toSessionItem).toList();
                    // 此处直接调用 emitter（list stream 无需。AgentEventBus。
