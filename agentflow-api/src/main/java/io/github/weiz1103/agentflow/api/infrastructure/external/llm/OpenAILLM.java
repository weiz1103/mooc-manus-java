package io.github.weiz1103.agentflow.api.infrastructure.external.llm;

import com.fasterxml.jackson.databind.ObjectMapper;
import io.github.weiz1103.agentflow.api.domain.external.LLM;
...
public class OpenAILLM implements LLM {
     ...
     * @return LLM返回的消息Map
     * @throws RuntimeException 调用失败时抛。
     */
    @Override
    ...
    public Map<String, Object> invoke(
        ...
        try {
            ...
            if (tools != null && !tools.isEmpty()) {
                logger.info("调用OpenAI客户端向LLM发起请求并携带工具信。 {}", modelName);
                requestBody.put("tools", tools);
                // 关闭并行工具调用(deepseek没有这个参数，其他模型可能需。
                requestBody.put("parallel_tool_calls", false);
                if (toolChoice != null) {
                    requestBody.put("tool_choice", toolChoice);
                }
            } else {
                // 3.未传递工具则删除tools/tool_choice等参。
                logger.info("调用OpenAI客户端向LLM发起请求未携带工。 {}", modelName);
            }
            ...
            logger.error("调用OpenAI客户端发生错。 {}", e.getMessage());
