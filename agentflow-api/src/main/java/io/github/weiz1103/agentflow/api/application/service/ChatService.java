...
import io.github.weiz1103.agentflow.api.application.agent.AgentRunner;
import io.github.weiz1103.agentflow.api.domain.external.Task;
import io.github.weiz1103.agentflow.api.domain.model.session.Session;
import io.github.weiz1103.agentflow.api.domain.external.TaskDispatchQueue;
...
import io.github.weiz1103.agentflow.api.infrastructure.event.AgentEventBus;
import io.github.weiz1103.agentflow.api.domain.exception.SessionBusyException;
import io.github.weiz1103.agentflow.api.domain.exception.SessionNotFoundException;
import io.github.weiz1103.agentflow.api.infrastructure.config.AppProperties;
...
import io.github.weiz1103.agentflow.api.infrastructure.observability.ExecutionObservationSink;
import io.github.weiz1103.agentflow.common.event.BaseEvent;
import io.github.weiz1103.agentflow.common.event.DoneEvent;
import io.github.weiz1103.agentflow.common.event.ErrorEvent;
import io.github.weiz1103.agentflow.common.event.MessageEvent;
import io.github.weiz1103.agentflow.common.event.WaitEvent;
 ...
 *   <li><b>断线续流</b>：{@link #resumeStream} 。。Redis Stream 重放未推送事。</li>
 * </ul>
 * </p>
 *
 * <p>本类只负责流程编排，不涉。AI 推理细节（由 {@link AgentRunner} 负责。
...
public class ChatService {
     ...
     * 此方法在虚拟线程中执行（。Controller 启动），可安全阻塞。</p>
     *
    ...
    public void sendMessage(String sessionId, String message, List<String> attachments,
        ...
        // 3. 确保 Task 。Sandbox 就绪，并将会话状态置。RUNNING
        ...
        if (!appProperties.getAgent().isAsyncExecutionEnabled()) {
            // 5A. 同步执行回退路径（保留兼容开关）
            log.info("开始同。Agent 执行: sessionId={}", sessionId);
        ...
        // 5B. 异步任务提交路径：提交命令给后台 worker，本请求只负。tail 输出。
        ...
        log.info("已提交后。Agent 任务: sessionId={}, taskId={}, dispatchMessageId={}",
                sessionId, task.getId(), dispatchMessageId);

        // 6. 当前连接改为从用户消息之后继续消费该 task 的输出事件，实现“提。+ 结果回推。
    ...
    public void resumeStream(String sessionId, String lastEventId, SsePublisher publisher) {
        ...
        if (task == null) {
            log.info("会话无进行中任务，续流直接返。 sessionId={}", sessionId);
            return;
        }

        log.info("开始断线续。 sessionId={}, lastEventId={}", sessionId, lastEventId);
        replayFromStream(session, task, lastEventId, publisher);
    }

    /**
     * 。Redis Stream 重放事件，直到遇到终止事件或流尽。
     *
     * <p>续流分两阶段。
     * <ol>
     *   <li>会话 RUNNING 中：阻塞等待新事件（最多等。5 分钟。</li>
     *   <li>会话已结束：非阻塞读取剩余事件后退。</li>
     * </ol>
     * </p>
     */
    private void replayFromStream(Session session, Task task, String lastEventId, SsePublisher publisher) {
        ...
        while (waitForNew && !publisher.isClientGone()) {
            String[] record = task.getOutputStream().get(nextId, 1000); // 1秒超。
            if (record == null) {
                if (++idleCount >= 300) { // 最多等。5 分钟
                    log.info("续流等待超时，退。 sessionId={}", session.getId());
                    break;
                }
                continue;
            }
            ...
            if (event != null) {
                ...
                nextId = record[0];
                log.warn("续流反序列化失败，跳过异常事。 id={}", nextId);
            }
        }
