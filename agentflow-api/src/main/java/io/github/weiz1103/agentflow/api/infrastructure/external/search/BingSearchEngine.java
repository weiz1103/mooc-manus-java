package io.github.weiz1103.agentflow.api.infrastructure.external.search;

import io.github.weiz1103.agentflow.api.domain.external.SearchEngine;
import io.github.weiz1103.agentflow.api.domain.model.search.SearchResultItem;
...
public class BingSearchEngine implements SearchEngine {
    ...
    public ToolResult<SearchResults> invoke(String query, String dateRange) {
        try {
            ...
            // 2.判断date_range是否存在并提取真实检索数。
            if (dateRange != null && !"all".equals(dateRange)) {
                // 3.获取当前日期的天数距。970-01-01的天。
            ...
            for (Element item : resultItems) {
                try {
                    ...
                    logger.warn("解析Bing搜索结果条目时出。 {}", e.getMessage());
                }
            }
