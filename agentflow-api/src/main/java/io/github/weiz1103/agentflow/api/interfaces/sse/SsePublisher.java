package io.github.weiz1103.agentflow.api.interfaces.sse;
...
public class SsePublisher {
    ...
    /**
     * @param emitter      Spring MVC 提供。SSE 发射。
     * @param objectMapper Spring 容器中已配置好多态序列化。ObjectMapper
     ...
     * 向客户端推送一。Agent 事件。
     *
     * @return false 表示客户端已断开，调用方可据此提前终。
     */
    public boolean publish(BaseEvent event) {
        ...
        try {
            ...
            log.warn("SSE 推送事件失。 type={}, error={}", resolveEventType(event), e.getMessage());
            return true; // 推送失败但客户端可能还在，不中断流。
        }
    }
