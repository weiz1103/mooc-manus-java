 ...
 * </p>
 *
 * <p><b>扩展点：</b> 注入不同。{@link BrowserPort} 实现（如 Selenium、Playwright）即可切换底层驱动。</p>
...
public class BrowserToolService {
    ...
    public String browserRestart(
            @ToolParam(description = "重启后要访问。URL") String url
    ) {
    ...
    public String browserInput(
            ...
            @ToolParam(description = "输入后是否按 Enter 。) Boolean pressEnter,
            @ToolParam(description = "元素索引（可选）", required = false) Integer index,
            @ToolParam(description = "输入。X 坐标（可选）", required = false) Double coordinateX,
            @ToolParam(description = "输入。Y 坐标（可选）", required = false) Double coordinateY
    ) {
    ...
    public String browserScrollUp(
            @ToolParam(description = "是否直接滚动到页面顶。, required = false) Boolean toTop
    ) {
    ...
    public String browserScrollDown(
            @ToolParam(description = "是否直接滚动到页面底。, required = false) Boolean toBottom
    ) {
    ...
    @Tool(name = "browser_console_view", description = "查看浏览器控制台输出，用于检。JavaScript 日志或调试页面错误。)
