 ...
 *
 * <p>
 * 功能：将用户的任。需求拆解成多个子步骤，根据已完成的子任务更新规划。
 * 特点。
 * - format = "json_object"：输。JSON 格式规划
...
""";
    ...
    /** 创建 Plan 规划提示词模板，内部。{message}+{attachments} 占位。*/
...
""";
    ...
    /** 更新 Plan 规划提示词模板，内部。{plan} 。{step} 占位。*/
...
""";
     ...
     * 3. 解析 JSON 。Plan 对象
     * 4. 发。PlanEvent(CREATED) 事件
     * </p>
     *
     * @param message 用户消息（含文本和附件路径）
     * @param emitter 事件发射。
     * @return 创建。Plan 对象，失败时返回 null
     */
    public Plan createPlan(io.github.weiz1103.agentflow.springai.model.AgentMessage message, Consumer<BaseEvent> emitter) {
     ...
     * <p>
     * 流程。
     * 1. 使用 plan + step 创建更新 Plan 提示。
     * 2. 调用 invoke 获取 JSON 响应
     * 3. 解析 JSON，更。plan.steps（保留已完成步骤，替换后续步骤）
     * 4. 发。PlanEvent(UPDATED) 事件
     * </p>
     *
    ...
    /**
     * 。Plan 领域模型转换。agentflow-common 。PlanEvent.PlanData（用。SSE 传输。
     */
