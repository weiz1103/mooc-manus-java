package io.github.weiz1103.agentflow.api.application.agent;
...
import io.github.weiz1103.agentflow.api.domain.service.agent.AgentStrategy;
 ...
 *
 * <p>异常兜底：无。AI 执行成功与否，都保证会话最终从 RUNNING 状态退出，
 * 防止会话永久锁死在执行中。</p>
 */
@Component
    ...
    public void run(Session session, Task task,
        ...
        try {
            ...
            agentStrategy.execute(session.getId(), message, attachments)
                    .doOnNext(event -> {
                        checkCancelled(taskId);
                        // 将事件写。Redis Stream（支持客户端断线后重连续流）
            ...
            log.info("Agent 执行被取。 sessionId={}, reason={}", session.getId(), e.getMessage());
