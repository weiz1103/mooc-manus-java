...
/**
 * 基于 ReAct 架构的执。Agent（Spring AI 实现）。
 *
 * <p>
 * 功能。
 * - execute_step：根据传递的消息+规划+子步骤，执行相应的子步骤
 * - summarize：汇总历史的消息并生成最终回。附件
 * </p>
...
""";
     ...
     * 流程。
     * 1. 根据传递的内容生成执行消息
     * 2. 更新步骤执行状态为 RUNNING 并发。StepEvent(STARTED)
     * 3. 调用 invokeLlm 获取执行结果（json_object 格式，允许工具调用）
     * 4. 如果 invokeLlm 返回 null，说明遇到了 WaitEvent（已。invokeLlm 发送），直接返。true
     * 5. 解析执行结果 JSON 。Step 对象
     * 6. 更新步骤状态，发。StepEvent(COMPLETED)
     * 7. 如果步骤有结果，发。MessageEvent
     * </p>
     *
     * @param plan    当前规划
     * @param step    当前要执行的子步。
     * @param message 用户消息
     * @param emitter 事件发射。
     * @return true 表示遇到。WaitEvent（需要等待用户输入），false 表示步骤正常完成
     */
    public boolean executeStep(Plan plan, Step step, AgentMessage message, Consumer<BaseEvent> emitter) {
        ...
        try {
            ...
            // 8. 已移。step.getResult() 的广播。
            // 因为在提示词中，LLM 已经被要求通过 message_notify_user 与用户通讯。
            // 如果这里再把内部使用。step.result 发送给用户，会导致界面出现重复消息。
        } catch (Exception e) {
        ...
        // 9. 循环迭代完成后代表子步骤已实现，更新状。
        step.setStatus(ExecutionStatus.COMPLETED);
        return false;
    }

    /**
     * 调用 Agent 汇总历史的消息并生成最终回。附件。
     *
     * <p>
     * 流程。
     * 1. 构建汇总请。query
     * 2. 调用 invoke 方法获取 Agent 生成的事。
     * 3. 判断事件类型是否是消息事件，如果是则表示 Agent 结构化生成汇总内。
     * 4. 解析输出内容
     * 5. 返回消息事件并将消息+附件进行相应
     * </p>
     *
    ...
    public void summarize(Consumer<BaseEvent> emitter) {
        ...
        try {
            ...
            logger.error("解析汇总结果失。 {}", e.getMessage());
            // 解析失败时直接返回原始内。
            emitter.accept(MessageEvent.builder()
                    .role("assistant")
    ...
    /**
     * 。Step 领域模型转换。agentflow-common StepEvent.StepData（用。SSE 传输。
     */
