...
import io.github.weiz1103.agentflow.springai.agent.SpringAIPlannerAgent;
import io.github.weiz1103.agentflow.springai.agent.SpringAIReActAgent;
...
import io.github.weiz1103.agentflow.springai.model.ExecutionStatus;
import io.github.weiz1103.agentflow.springai.model.Plan;
import io.github.weiz1103.agentflow.springai.model.Step;
 ...
 * 顺序。
 * 1. PlannerAgent 生成规划。
 * 2. 循环取出规划中的子步骤，。ReActAgent 执行，依次迭代；
 * 3. ReActAgent 执行完每一个子步骤之后，需要将子步骤结。Plan传递给PlannerAgent让其更新规划。
 * 4. 循环取出规划中的子步骤，。ReActAgent 执行，依次迭代；
 * 5. ...
 * 6. 直到所有子任务/步骤都完成，这时候将子步骤的所有结果汇总进行总结(ReActAgent)。
 * </p>
 *
 * <p><b>扩展点：</b>
 * - 可替。AgentMemoryStore 实现（如 DB 持久化）
 * - 可替。SessionStateLoader 实现
...
public class SpringAIPlannerReActFlow {
     ...
     * 构造函数，完成规划与执行流的初始化。
     *
     * @param chatClient          Spring AI ChatClient（已配置。model 等参数）
     * @param toolCallbacks       工具回调列表（FileTool、ShellTool、BrowserTool 等）
     * @param memoryStore         记忆持久化存。
     * @param sessionStateLoader  会话状态加。更新接口
    ...
    /**
     * 传递消息，运行流，在六步调。planner&react 智能体组合完成任务并返回对应事件。
     *
    ...
    private void runFlow(String sessionId, String message, List<String> attachPaths, Consumer<BaseEvent> emitter) {
        ...
        // 2. 判断会话的状态是不是空闲
        //    如果不是则有可能有两种状。
        //     - 任务未结束，还在运行，但是用户又传递了一条消。
        //     - Agent 在等待人类输入，这时候人类输入了
        //    这时候均需要处理历史消息列表，避免AI(工具调用消息)后直接接上人类消。
        ...
        while (true) {
            ...
            if (flowStatus == FlowStatus.IDLE) {
                ...
                // 17. 流的状态为执行中，先将规划状态调整为运行中，同时调用执行Agent完成每个子步。
                if (plan != null) {
                    plan.setStatus(ExecutionStatus.RUNNING);
                }

                // 18. 获取当前规划的下一个需要执行的子步。
                ...
                // 27. 规划状态已完成则更。plan 状态，并发送规划事件通知API已完。
                if (plan != null) {
    ...
    /**
     * 创建 PlannerAgent 实例（每。invoke 调用时创建）。
    ...
    /**
     * 创建 ReActAgent 实例（每。invoke 调用时创建）。
    ...
    /**
     * 。SessionStateLoader 加载最。Plan（JSON 反序列化。
     */
    ...
    /**
     * 。Plan 序列化后保存。SessionStateLoader
     */
    ...
    /**
     * 。Plan 转换。agentflow-common PlanEvent.PlanData（用。SSE 传输。
     */
