package io.github.weiz1103.agentflow.api.infrastructure.task;
 ...
 * 后台 Agent 任务 Worker。
 *
 * <p>职责：从任务分发队列拉取一。AgentTaskCommand，在后台线程中执行完整的
 * Agent 流程。这。Controller 线程只负责提交与订阅输出，不再直接承载完整执行。</p>
 */
@Component
    ...
    private void loop() {
        while (running.get() && !Thread.currentThread().isInterrupted()) {
            try {
                ...
                log.error("后台 Agent Worker 主循环异。 {}", e.getMessage(), e);
                safeSleep(300L);
            }
        }
    }
