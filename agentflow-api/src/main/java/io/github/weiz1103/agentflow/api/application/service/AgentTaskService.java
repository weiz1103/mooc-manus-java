 ...
 * Task 。Agent 和前端之间的事件中转通道，基。Redis Stream 实现。</p>
 *
 * <p>职责边界：此类属于应用服务层，可以持有基础设施依赖（RedisTaskFactory），
 * 但对外只暴露领域接口 {@link Task}，屏。Redis 实现细节。</p>
 */
@Service
public class AgentTaskService {
    ...
    /**
     * 确保会话拥有一个就绪的 Task 。Sandbox。
     *
     * <p>首次执行时创建新沙箱和新 Task；后续执行复用已有资源。
     * 方法执行。session 。taskId 。sandboxId 已更新，调用方负责持久化。</p>
     *
     * @return 就绪。Task
     */
    @Transactional
    public Task ensureTask(Session session) {
        ...
        var sandbox = Optional.ofNullable(session.getSandboxId())
                ...
                .orElseGet(() -> {
                    ...
                    log.info("沙箱已创。 sandboxId={}, sessionId={}", newSandbox.getId(), session.getId());
                    return newSandbox;
                });

        // 每轮对话创建。Task（Redis Stream 管道。
        RedisStreamTask task = taskFactory.createTask(session.getId(), sandbox);
        session.setTaskId(task.getId());
        sessionRepository.save(session);

        log.info("Task 已就。 taskId={}, sessionId={}", task.getId(), session.getId());
        return task;
    }
}
