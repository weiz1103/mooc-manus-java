package io.github.weiz1103.agentflow.api.domain.model.context;
...
/**
 * 智能体运行时上下。(Agent Context)
 * 迁移。Python 项目中的 Context 字典机制。
 * 核心逻辑：利。Record 的不可变性保证基础属性安全，利用 ConcurrentHashMap 保证共享状态的并发安全。
 *
 * @param sessionId    会话唯一标识（对应数据库中的 Session ID。
 * @param traceId      全链路追。ID（用于日志排查）
